...
#        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=f"You don't have permission to access this server")
#    user = db.query(models.User).filter(models.User.id == token.id).first()
#    return user
#    
#
#def get_current_worker(token: str = Depends(oauth2_scheme), db: Session = Depends(database.get_db)):
#    credentials_exception = HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail=f"Could not validate credentials", headers={"WWW-Authenticate": "Bearer"}) 
#
#    token = verify_access_token(token, credentials_exception)
#    if not token.role == 'worker':
#        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=f"You don't have permission to access this server")
#    user = db.query(models.User).filter(models.User.id == token.id).first()
#    return user
